...
_NUM_BASIS = 6  # 6 × 15-min piecewise time intervals
...
class IntervalData:
    ...
    basis_idx: int  # which of the 6 basis functions
...
def _time_to_basis(t: float, alpha_1: float = 0.0) -> int:
    """Map match minute to basis function index [0, 5].
    ...
    Basis boundaries (with stoppage):
        ...
        5: [75+α₁, T_m)    — late second half + stoppage
    """
    ht = 45.0 + alpha_1
    if t < 15.0:
        return 0
...
class MMPPModel(nn.Module):
    ...
    Learnable parameters:
        ...
        b: (6,) time-profile basis coefficients
    ...
    def __init__(
        ...
        # Time profile: 6 basis coefficients, init = 0
